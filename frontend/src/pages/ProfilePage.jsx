  ...
  return (
    <main className="min-h-screen text-white" style={{ background: "var(--bg)" }}>
      ...
      <div className="fixed inset-0 -z-10">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_30%_0%,rgba(124,58,237,0.08),transparent_40%)]" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#05060b] via-[#020306] to-[#030509]" />
      </div>

      {/* Profile Container */}
      <div className="mx-auto max-w-[1000px] px-4 pt-24 pb-12 sm:px-6">
        
        {/* ── 1. Header Banner ── */}
        <section className="relative overflow-hidden rounded-3xl border border-white/[0.06] bg-white/[0.02] p-8 mb-8 shadow-2xl shadow-black/40 backdrop-blur-md">
          ...
          <div className="relative flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="relative shrink-0">
              <div className="h-28 w-28 sm:h-32 sm:w-32 overflow-hidden rounded-full ring-4 ring-violet-500/20 ring-offset-4 ring-offset-[#0b0d16] shadow-2xl shadow-violet-900/30">
                <img
              ...
              {/* Online indicator */}
              <span className="absolute bottom-2 right-2 h-5 w-5 rounded-full border-4 border-[#0b0d16] bg-emerald-500 shadow-lg shadow-emerald-500/40" />
            </div>
            ...
            <div className="flex-1 text-center md:text-left flex flex-col items-center md:items-start">
              <div className="flex flex-col md:flex-row md:items-center justify-between w-full gap-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-zinc-500 mt-1">@{profile.username}</p>
                </div>
                ...
                <div className="shrink-0 mt-4 md:mt-0">
                  {isOwnProfile ? (
                    <button
                      ...
                      className="flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-6 py-2.5 text-sm font-bold text-violet-300 transition-all hover:bg-violet-500/20 hover:text-violet-200 hover:border-violet-400/50 hover:scale-105"
                    >
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center min-w-[120px] rounded-full px-6 py-2.5 text-sm font-bold transition-all duration-300 ${
                        isFollowingTarget
                          ? "border border-white/[0.15] bg-white/[0.05] text-zinc-300 hover:bg-rose-500/10 hover:text-rose-400 hover:border-rose-500/30"
                          : "bg-violet-600 text-white shadow-xl shadow-violet-900/40 hover:-translate-y-0.5 hover:bg-violet-500 hover:shadow-violet-900/60"
                      }`}
                    >
              ...
              {/* Network Stats & Join Date merged */}
              <div className="mt-6 flex flex-wrap items-center justify-center md:justify-start gap-6 text-sm">
                <button onClick={openFollowersModal} className="group flex items-center gap-1.5 transition-all">
                  <span className="font-black text-white group-hover:text-violet-400">{profile.followers.length}</span>
                  <span className="text-zinc-500 font-medium group-hover:text-zinc-400">Followers</span>
                </button>
                <button onClick={openFollowingModal} className="group flex items-center gap-1.5 transition-all">
                  <span className="font-black text-white group-hover:text-violet-400">{profile.following.length}</span>
                  <span className="text-zinc-500 font-medium group-hover:text-zinc-400">Following</span>
                </button>
        ...
        {/* ── 2. Content Tabs (Sleek Minimalist) ── */}
        <div className="flex items-center gap-8 border-b border-white/[0.08] mb-8 overflow-x-auto [scrollbar-width:none]">
          {[
            ...
            >
              {tab.label}
              {activeTab === tab.id && (
                <span className="absolute bottom-0 left-0 h-[3px] w-full bg-violet-500 rounded-t-full shadow-[0_0_10px_rgba(124,58,237,0.5)]" />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="w-full">
          ...
          {activeTab === "reviews" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {localReviews.length === 0 ? (
                <div className="col-span-full rounded-3xl border border-white/[0.06] bg-white/[0.02] p-16 text-center">
                  <div className="mb-4 grid h-16 w-16 mx-auto place-items-center rounded-2xl border border-white/[0.07] bg-white/[0.03]">
                  ...
                  <p className="text-lg font-black text-white">No reviews yet</p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex gap-4 mb-3">
                        <Link to={route} className="w-16 h-24 rounded-xl overflow-hidden shrink-0 border border-white/[0.07] bg-zinc-900 shadow-md block">
                          {media?.posterPath ? (
                            <img src={posterUrl(media.posterPath)} alt={media.title} className="h-full w-full object-cover transition hover:scale-105 duration-300" />
                          ) : (
                            <div className="h-full w-full flex items-center justify-center text-[10px] text-zinc-700 font-medium">Loading...</div>
                          )}
                        </Link>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {userCollections.length === 0 ? (
                <div className="col-span-full rounded-3xl border border-white/[0.06] bg-white/[0.02] p-16 text-center">
                  ...
                  {!isOwnProfile && <p className="text-sm text-zinc-500 mt-2">This user hasn't made any collections public yet.</p>}
                </div>
              ) : (
                userCollections.map((col) => {
                  ...
                  return (
                    <Link
                      ...
                      className="group relative overflow-hidden rounded-2xl border border-white/[0.06] bg-zinc-900 shadow-xl shadow-black/30 transition-all hover:-translate-y-1 hover:border-violet-500/50 hover:shadow-violet-500/30"
                      style={{ aspectRatio: "16/9" }}
                    >
                      {/* Background Image */}
                      {bgImage ? (
                        <img src={bgImage} alt={col.name || col.title} className="absolute inset-0 h-full w-full object-cover opacity-70 transition-transform duration-700 group-hover:scale-110 group-hover:opacity-100" />
                      ) : (
                      ...
                      {/* Gradient Overlay */}
                      <div className="absolute inset-0 bg-gradient-to-t from-[#050507] via-[#050507]/50 to-transparent transition-opacity duration-500 group-hover:opacity-90" />
                      {/* Public Badge */}
                      {isOwnProfile && col.isPublic && (
                        <div className="absolute top-2 right-2 z-10">
                          <span className="rounded-full bg-emerald-500/20 px-2 py-0.5 text-[8px] font-black uppercase tracking-[0.1em] text-emerald-400 border border-emerald-500/30 backdrop-blur-md">Public</span>
                        </div>
                      )}
                      {/* Content */}
                      <div className="absolute inset-0 flex flex-col justify-end p-4 z-10">
                        <h3 className="text-base font-black text-white leading-tight drop-shadow-md line-clamp-1 mb-1.5">{col.name || col.title}</h3>
          ...
          {activeTab === "clubs" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {userClubs.length === 0 ? (
                <div className="col-span-full rounded-3xl border border-white/[0.06] bg-white/[0.02] p-16 text-center">
                  <p className="text-lg font-black text-white">No clubs joined yet</p>
                </div>
              ) : (
                userClubs.map((club) => (
                  <Link
                    ...
                    className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4 shadow-md transition hover:-translate-y-0.5 hover:bg-white/[0.04] hover:border-white/[0.1] backdrop-blur-sm"
                  >
                    <div className="flex items-center gap-3">
                      <div className="h-12 w-12 rounded-xl bg-violet-500/15 shrink-0 overflow-hidden border border-white/10">
                        {club.coverImage ? (
