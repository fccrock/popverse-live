...
import { posterUrl, getTitle, getYear, getRating } from "../utils/tmdb";
...
function AnimeCard({ item }) {
  return (
    ...
    >
      <div className="relative aspect-[2/3] w-full overflow-hidden rounded-xl bg-zinc-900 shadow-xl ring-1 ring-white/10 transition-shadow duration-300 group-hover:shadow-2xl group-hover:shadow-rose-900/30">
        <img
      ...
      <div className="px-1">
        <h3 className="truncate text-base font-bold text-white group-hover:text-rose-400 transition-colors">
          {getTitle(item)}
        </h3>
...
function AnimeGrid({ title, items, icon, limit }) {
  if (items.length === 0) {
    return (
      <div className="mb-14 px-6 sm:px-10 lg:px-14 xl:px-16">
        ...
        <p className="text-zinc-500">No anime found matching these filters.</p>
      </div>
    );
  }
    ...
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold transition-all backdrop-blur-md outline-none ${
          isOpen
            ? "border-rose-500/50 bg-white/[0.06] text-white ring-1 ring-rose-500/50"
            : "border-white/[0.08] bg-black/40 text-zinc-300 hover:border-rose-500/40 hover:bg-white/[0.04] hover:text-white"
        }`}
      >
        {displayLabel}
        <svg
          className={`h-4 w-4 transition-transform duration-200 ${isOpen ? "rotate-180 text-rose-400" : "opacity-70"}`}
...
export default function AnimePage() {
  ...
  useEffect(() => {
    api.getTrendingAnime().then((data) => setTrending(data.results || []));
    api.getTopRatedAnime().then((data) => setTopRated(data.results || []));
  }, []);
  ...
  const filteredTrending = useMemo(() => filterAndSort(trending), [trending, ratingFilter, yearFilter, sortFilter]);
  const filteredTopRated = useMemo(() => filterAndSort(topRated), [topRated, ratingFilter, yearFilter, sortFilter]);

  return (
    <main className="min-h-screen text-white pb-24 relative z-0">
      {/* Premium Background — deep crimson/dark red top gradient for Anime */}
      <div className="fixed inset-0 -z-10 bg-[#050505]">
        <div className="absolute top-0 left-0 right-0 h-[600px] bg-gradient-to-b from-[#2a0a10] via-[#160408] to-transparent opacity-90" />
      </div>
      ...
      <div className="mx-auto max-w-[1840px] pt-24 sm:pt-32">
        ...
        <div className="mb-10 px-6 sm:px-10 lg:px-14 xl:px-16">
          ...
          <p className="mt-3 max-w-2xl text-sm sm:text-base font-medium text-zinc-300">
            From shonen epics to psychological masterpieces.
          </p>
        </div>
