...
import { posterUrl, backdropUrl } from "../utils/tmdb";
...
function runtime(minutes) { return minutes?.[0] ? `${minutes[0]}m episodes` : "Runtime TBA"; }
...
function getTrailer(videos) {
  ...
  const officialTrailers = yt.filter((v) => v.official && v.type === "Trailer" && !isBad(v));
  if (officialTrailers.length) return officialTrailers.sort(byQuality)[0];
  const officialTeasers = yt.filter((v) => v.official && v.type === "Teaser" && !isBad(v));
  if (officialTeasers.length) return officialTeasers.sort(byQuality)[0];
  return yt.filter((v) => v.type === "Trailer" && !isBad(v)).sort(byQuality)[0];
}

function getProviders(show) {
  const results = show?.["watch/providers"]?.results;
  if (!results) return [];
...
function isAnime(show) {
  return (
    show.genres?.some((g) => g.id === animeGenreId) ||
    show.origin_country?.includes("JP") ||
    /anime/i.test(show.name)
  );
}

function getVibes(show, anime) {
  const tags = (show.genres ?? []).map((g) => g.name).slice(0, 4);
  if (anime) tags.unshift("Anime");
  if (show.vote_average >= 8) tags.push("Fan Favorite");
  if (show.status === "Returning Series") tags.push("Currently Airing");
  tags.push(anime ? "Stylized" : "Binge Worthy");
  return [...new Set(tags)].slice(0, 7);
}

export default function TvDetailPage() {
  const { id } = useParams();
  const { data: show, loading, error } = useFetch(() => api.getTvDetails(id), [id]);
  ...
  if (error || !show) {
    return (
      <main className="grid min-h-screen place-items-center bg-[#030509] px-5 text-white">
        <div className="max-w-xl rounded-2xl border border-white/[0.07] bg-white/[0.03] p-10 text-center">
          <h1 className="text-3xl font-black">TV show unavailable</h1>
          <p className="mt-3 text-zinc-500 text-sm">{error ?? "Check your TMDb token and try again."}</p>
          <Link className="mt-7 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-6 py-3 text-sm font-bold shadow-lg shadow-violet-900/30 transition hover:-translate-y-0.5 hover:bg-violet-500" to="/search">← Back to search</Link>
        </div>
      </main>
    );
  }

  const anime     = isAnime(show);
  const trailer   = getTrailer(show.videos);
  const providers = getProviders(show);
  const cast      = show.credits?.cast.slice(0, 12) ?? [];
  const vibes     = getVibes(show, anime);
  const language  = show.spoken_languages?.[0]?.english_name ?? "Unknown";
  const creator   = show.created_by?.[0]?.name ?? show.networks?.[0]?.name ?? "Unknown";
  const studio    = show.production_companies?.[0]?.name ?? show.networks?.[0]?.name ?? "Unknown";
  const country   = show.origin_country?.[0] ?? "Global";

  const accent      = anime ? "cyan" : "violet";
  const accentText  = anime ? "text-cyan-300" : "text-violet-300";
  const accentEyebrow = anime ? "text-cyan-400" : "text-violet-400";
  const buttonClass = anime
  ...
  const buttonActiveClass = anime
    ? "bg-cyan-400 text-black shadow-cyan-900/40 hover:bg-cyan-300"
    : "bg-violet-500 text-white shadow-violet-900/40 hover:bg-violet-400";

  // ── Build the media item object for collections ─────────────────────────────
  const mediaItem = {
    mediaId:    `tv-${id}`,
    mediaType:  anime ? "anime" : "tv",
    title:      show.name,
  ...
  return (
    <main className="min-h-screen text-white" style={{ background: "var(--bg)" }}>
      ...
      <div className="fixed inset-0 -z-10">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_12%_0%,rgba(124,58,237,0.14),transparent_30%),radial-gradient(ellipse_at_86%_0%,rgba(6,182,212,0.10),transparent_30%)]" />
      ...
      <section className="relative min-h-[660px] overflow-hidden">
        {show.backdrop_path && (
          <img alt="" className="absolute inset-0 h-full w-full object-cover opacity-75" src={backdropUrl(show.backdrop_path)} />
        )}
        ...
        <div className={`absolute inset-0 ${anime ? "bg-[radial-gradient(ellipse_at_68%_40%,rgba(6,182,212,0.12),transparent_40%)]" : "bg-[radial-gradient(ellipse_at_68%_40%,rgba(124,58,237,0.12),transparent_40%)]"}`} />
        <div className="absolute inset-x-0 bottom-0 h-64 bg-gradient-to-t from-[#030509] to-transparent" />

        <TrailerModal accent={accent} title={show.name} trailerKey={trailer?.key} />
        ...
        <div className="relative mx-auto grid min-h-[660px] max-w-[1600px] items-end gap-10 px-5 pb-16 pt-28 sm:px-8 lg:grid-cols-[240px_minmax(0,1fr)_360px] lg:px-16 xl:px-20">
          ...
          <div className="w-36 overflow-hidden rounded-2xl border border-white/[0.08] bg-zinc-900 shadow-2xl shadow-black/80 ring-1 ring-white/[0.05] sm:w-48 lg:w-full">
            {show.poster_path
              ? <img alt="" className="aspect-[2/3] h-full w-full object-cover" src={posterUrl(show.poster_path)} />
          ...
          <div className="max-w-4xl pb-2 animate-fade-up">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className={`text-[11px] font-bold uppercase tracking-[0.25em] ${accentEyebrow}`}>{anime ? "Anime" : "TV Show"}</span>
              <span className="text-zinc-700">·</span>
              <span className="text-[11px] font-semibold text-zinc-500">{year(show.first_air_date)}</span>
              <span className="text-zinc-700">·</span>
              <span className="text-[11px] font-semibold text-zinc-500">{show.number_of_seasons} seasons</span>
              <span className="text-zinc-700">·</span>
              <span className="text-[11px] font-semibold text-zinc-500">{runtime(show.episode_run_time)}</span>
            </div>
            <h1 className="text-4xl font-black leading-tight tracking-tight text-white sm:text-5xl lg:text-6xl">{show.name}</h1>
            {anime && show.original_name !== show.name && (
              <p className={`mt-2 text-lg font-bold ${accentText}`}>{show.original_name}</p>
            )}

            {/* Metadata grid */}
            <div className="mt-8 grid max-w-4xl grid-cols-2 gap-x-8 gap-y-5 sm:grid-cols-3">
              {[
                { label: "Rating", value: rating(show.vote_average), color: "text-yellow-300" },
                { label: "Country", value: country, color: "text-white" },
                { label: "Language", value: language, color: "text-white" },
                { label: "Episodes", value: show.number_of_episodes, color: "text-white" },
                { label: anime ? "Studio" : "Showrunner", value: anime ? studio : creator, color: "text-white" },
                { label: "Status", value: show.status, color: "text-white" },
              ].map(({ label, value }) => (
                <div key={label}>
          ...
          <aside className="space-y-2.5 self-end pb-2">
            {/* Watched toggle */}
            <button
              className={`flex w-full items-center justify-center gap-2.5 rounded-xl px-5 py-3.5 text-sm font-bold shadow-xl transition-all duration-200 hover:-translate-y-0.5 ${isWatched ? buttonActiveClass : buttonClass}`}
      ...
      <section className="mx-auto grid max-w-[1600px] gap-12 px-5 pb-28 sm:px-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-16 xl:px-20">
        <div className="min-w-0">
          ...
          <div className="flex flex-wrap gap-2">
            <span className={`rounded-lg border border-white/[0.08] bg-white/[0.04] px-3.5 py-1.5 text-xs font-bold shadow-lg shadow-black/20 ${accentText}`}>
              {anime ? "Anime Select" : "Popverse Select"}
            </span>
            <span className="rounded-lg border border-white/[0.08] bg-white/[0.04] px-3.5 py-1.5 text-xs font-bold text-zinc-300 shadow-lg shadow-black/20">{show.number_of_episodes} Episodes</span>
            <span className="rounded-lg border border-white/[0.08] bg-white/[0.04] px-3.5 py-1.5 text-xs font-bold text-zinc-300 shadow-lg shadow-black/20">{show.status}</span>
          </div>

          {/* Overview */}
          <section className="mt-10">
            <h2 className="text-2xl font-black tracking-tight text-white">Overview</h2>
            <p className="mt-5 max-w-5xl text-base leading-8 text-zinc-400">{show.overview || "No overview available."}</p>
          ...
          {cast.length > 0 && (
            <section className="mt-12 border-t border-white/[0.06] pt-10">
              <div className="mb-7 flex items-end justify-between gap-4">
                <h2 className="text-2xl font-black tracking-tight text-white">{anime ? "Voice Cast" : "Cast"}</h2>
              ...
              <div className="-mx-5 flex gap-8 overflow-x-auto px-5 pb-6 [scrollbar-width:thin] [scrollbar-color:rgba(124,58,237,0.3)_transparent]">
                {cast.map((member) => (
                  <article className="group/cast w-28 shrink-0 text-center" key={member.id}>
                    <div className={`mx-auto h-28 w-28 overflow-hidden rounded-full border border-white/[0.07] bg-zinc-900 shadow-xl shadow-black/30 ring-1 ring-white/[0.04] transition-all duration-300 group-hover/cast:-translate-y-1.5 ${anime ? "group-hover/cast:border-cyan-300/50" : "group-hover/cast:border-violet-400/50"}`}>
                      {member.profile_path
        ...
        <aside className="space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="mt-6 grid place-items-center">
              <div className={`grid h-52 w-52 place-items-center rounded-full p-7 shadow-2xl shadow-black/50 ${anime ? "bg-[conic-gradient(#22d3ee_0_42%,#a78bfa_42%_62%,#f472b6_62%_82%,#facc15_82%_100%)]" : "bg-[conic-gradient(#06b6d4_0_42%,#7c3aed_42%_62%,#f97316_62%_82%,#e11d48_82%_100%)]"}`}>
                <div className="grid h-full w-full place-items-center rounded-full bg-[#0e1018] text-center">
                  <div>
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-wider">{show.genres?.[0]?.name ?? (anime ? "Anime" : "TV")}</p>
                    <p className="mt-1 text-3xl font-black text-white">{Math.round(show.vote_average * 10)}%</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center justify-between" key={tag}>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-1.5 w-16 rounded-full bg-white/[0.07] overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-700 ${anime ? "bg-gradient-to-r from-cyan-500 to-violet-500" : "bg-gradient-to-r from-violet-500 to-rose-500"}`} style={{ width: `${45 - i * 8}%` }} />
                    </div>
              ...
              <div className="mt-5 space-y-2">
                {providers.slice(0, 4).map((provider) => (
                  <a
                    ...
                    href={getProviderLink(show.name, provider.provider_name)}
